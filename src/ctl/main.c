...
const titaniactl_mode modes[] = { { "report", titaniactl_mode_report, titaniactl_mode_report_json, "print the input report of connected controllers", nullptr },
	...
	{ "profile export", nullptr, nullptr, "export a controller profile to json", "{all, triangle, square, cross, circle, 0, 1, 2, 3} path/to/directory" },
	...
	if (argc > 1) {
		for (int i = 1; i < argc; ++i) {
			...
			if (strcmp(text, "-h") == 0 || strcmp(text, "--help") == 0 || strcmp(text, "help") == 0) {
				...
				printf("\t-z, --blocking: blocking reads (slower)\n");
			...
			if (strcmp(text, "-d") == 0 || strcmp(text, "--device") == 0) {
				...
				calibrate = false;
			} else if (strcmp(text, "-z") == 0 || strcmp(text, "--blocking") == 0) {
				blocking = true;
			} else if (text[0] != '-') {
				mode = text;
				if (argc - i > 1) {
