...
void titania_bt_loop_inner(const titania_handle handle) {
	while (1) {
		...
		dualsense_bt_cmd_packet_142 packet_0x11 = { 0 };
		packet_0x11.packet = DUALSENSE_BT_REPORT_HAPTICS_SETUP | DUALSENSE_BT_REPORT_MASK_HAS_LENGTH;
		packet_0x11.sized.length = 7;
		packet_0x11.sized.data[0] = 0xFE;
		packet_0x11.sized.data[5] = 0xFF;
		packet_0x11.sized.data[6] = state[handle].haptics.bt_timer.frame++;

		dualsense_bt_cmd_packet_142 packet_0x12 = { 0 };
		packet_0x12.packet = DUALSENSE_BT_REPORT_HAPTICS_GRANULE | DUALSENSE_BT_REPORT_MASK_HAS_LENGTH;
		packet_0x12.sized.length = DUALSENSE_SAMPLE_SIZE;
		if (resample_48000_to_3000(handle, (int8_t*) &packet_0x12.sized.data[0]) > 0) {
			dualsense_bt_cmd_142 cmd = { 0 };
			cmd.report_id = DUALSENSE_REPORT_BLUETOOTH_BIDIRECTIONAL_142;
			uint8_t* data_ptr = cmd.data;
			memcpy(data_ptr, &packet_0x11, packet_0x11.sized.length + 2);
			data_ptr += packet_0x11.sized.length + 2;
			memcpy(data_ptr, &packet_0x12, packet_0x12.sized.length + 2);
