...
void json_object_get_edge_stick(struct json* obj, const char* key, titania_edge_stick* data) {
	struct json* stick_obj = json_get(obj, key);
...
void json_object_get_edge_trigger(struct json* obj, const char* key, titania_edge_trigger* data) {
...
titaniactl_error titaniactl_mode_edge_import(titania_profile_id profile, struct json* data, titania_hid handle) {
	...
	struct json* stick_obj = json_get(data, "sticks");
	...
	struct json* trigger_obj = json_get(data, "triggers");
	...
	struct json* disabled_button_obj = json_get(data, "disabledButtons");
